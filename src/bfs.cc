...
int main(int argc, char* argv[]) {
  ...
  Builder b(cli);
  Graph g = b.MakeGraph();
  std::string src_filename = cli.src_filename();
  SourcePicker<Graph> *sp;
  SourcePicker<Graph> *vsp;

  if (src_filename != "") {
    std::ifstream file(src_filename);
    std::vector<NodeID> src_list;
    NodeID a;
    while (file >> a) {
      src_list.push_back(a);
    }
    sp = new SourcePicker<Graph>(g, src_list);
    vsp = new SourcePicker<Graph>(g, src_list);
  } else {
    sp = new SourcePicker<Graph>(g);
    vsp = new SourcePicker<Graph>(g);
  }
